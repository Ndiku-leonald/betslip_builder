...
def persist_live_match_snapshot(db: Session, state: LiveMatchState) -> LiveMatchSnapshot:
    ...
    row = LiveMatchSnapshot(
        ...
        halftime_away_score=state.halftime_away_score, statistics=state.statistics, events=list(state.events), auxiliary=state.auxiliary,
    )
    db.add(row)
    db.flush()
    return row
