...
def run() -> None:
    ...
    with Session(engine) as db:
        ...
        away_handicap = _market(fixture.id, "handicap", "win", 2.0, current, participant="away", line=.5)
        team_total = _market(fixture.id, "team_total", "over", 1.9, current, participant="home", line=1.5)
        persist_market_snapshots(db, prices + [old_home] + incomplete + whole_total + [away_handicap, team_total])
        latest = latest_market_snapshots(db, fixture.id); history = market_snapshot_history(db, fixture.id)
        groups = {}
        for item in latest: groups.setdefault((item.bookmaker, item.market_family, item.market_type, item.period, item.participant, item.line), []).append(item)
        service = MarketValueService(); values = []
        for item in latest:
            group = groups[(item.bookmaker, item.market_family, item.market_type, item.period, item.participant, item.line)]
            structure = {"win_probability": .45, "push_probability": .2, "loss_probability": .35, "calibration_status": "fitted"} if item.market_family == "totals" and item.line == 2.0 else {"win_probability": .62 if item.selection == "home" else .2 if item.selection == "draw" else .18, "push_probability": 0, "loss_probability": .38 if item.selection == "home" else .8 if item.selection == "draw" else .82, "calibration_status": "fitted"}
            values.append(service.evaluate(item, model_probability_structure=structure, market_group=group, confidence=78, data_quality=82, market_reliability=80, source_reliability=75, provider_agreement=1.0))
