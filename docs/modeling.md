...
`python -m app.training.train --sport football` and the basketball equivalent create a `candidate` model version. Models are never auto-promoted. Dataset splits are chronological and atomic by exact `data_cutoff_at`: a simultaneous kickoff group cannot cross train, validation, or test. Walk-forward evaluation fits a model on the past, fits sigmoid calibration on a validation window only, and evaluates on a later test window. Baselines are fitted on each fold's training history and scored on exactly the same held-out fixture IDs as the candidate. The football league-average baseline is an independent Poisson score model, not an outcome-frequency shortcut.

Production candidates persist calibrator parameters and per-market status, so inference exposes raw and calibrated probabilities. Mutually exclusive markets are normalized after calibration; complementary markets derive the opposite side from `1-p`. Reports include proper multiclass 1X2 Brier/log loss, family-level binary Brier/log loss/ECE, representative handicap metrics, baseline comparability metadata, and score error where applicable. Confidence is a separate higher-is-better evidence score: data quality, sample depth, calibration quality, competition coverage, distribution certainty, and model stability are recorded individually. Synthetic tests are not production performance claims.
