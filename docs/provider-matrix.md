...
| The Odds API | Odds context | Odds events, not canonical fixtures | NO | NO | NO | NO | Current/live event list only | YES for documented soccer markets where available | YES for documented live/upcoming markets where available | Bookmakers and markets depend on sport, region and plan | Upstream quota; current adapter needs persistent usage instrumentation | Implemented optional odds adapter |
