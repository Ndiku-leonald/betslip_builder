...
## Real data activation — final bounded run — 2026-09-26

- The current API-Football request returned **1,186** real fixtures for `2026-09-26` and canonical ingestion remained idempotent at **4,360** fixtures. The free-plan date restriction rejected the requested October window; no repeated future-date requests were made. The exact canonical Premier League competition (`provider_id=39`) currently covers **380** fixtures from **2024-08-16** through **2025-05-25**, so it cannot overlap the October Odds API event window yet.
- API-Football local accounting is **92** outbound attempts today against the free-mode 100-attempt safety budget. football-data.org is at its configured **10**-request quota. API-Basketball has one recorded request and its previous real request remained an HTTP 403 entitlement failure.
- The Odds API authenticated successfully. A real `soccer_epl` request returned **20** events from **2026-10-10** through **2026-10-19**, with **488** requests remaining in the provider response header. Strict competition/team/kickoff reconciliation matched **0/20** to canonical API-Football fixtures; **0** snapshots were stored. Odds events were not promoted to canonical fixtures.
- A controlled API-Football fixture-detail statistics request for provider fixture `1208021` returned HTTP 200. The nested `response[0].statistics` payload was normalized and persisted as **2** real team-stat rows. The parser now supports both nested fixture-detail and flat statistics responses; the remaining historical corpus is still not statistically complete.
- Computed real feature rows: **4,360**; persisted feature snapshots: **4,358**. Persisted production predictions: **0**. Model versions remain candidates only; no champion was promoted because Poisson and Dixon–Coles did not clear the chronological Elo non-regression gate.
- The real-only lineage gate now requires a non-synthetic canonical fixture, linked feature snapshot, prediction backed by a `champion` model, and non-synthetic odds snapshot. The lineage regression test passes. All 3.00, 5.00 and 10.00 builder targets returned `NO_SAFE_TARGET` with **0** candidates and **0** slips.
- Live and basketball prediction activation remain unclaimed: no live call was made in the final bounded smoke, and API-Basketball entitlement/statistics coverage is not sufficient for a production recommendation.
