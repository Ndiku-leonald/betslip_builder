...
from app.models import Fixture, Team, TeamMatchStatistic
...
async def run(args: argparse.Namespace) -> int:
    ...
    football_fixture_request_blocked = False
    today_items = []
    try:
        ...
        football_fixture_request_blocked = True
    if not football_fixture_request_blocked:
        report("API-Football", "CONFIGURED")
        report("Today fixtures", "PASS", f"{len(today_items)} returned")

    date_results: dict[str, list] = {_day(0): today_items} if not football_fixture_request_blocked else {}
    if not football_fixture_request_blocked:
    ...
    if real_items:
        ...
        report("Canonical fixture ingestion", "SKIPPED", "fresh API-Football fixture request unavailable; existing canonical rows retained")
    ...
    if completed:
        report("Historical/statistics detail", "PASS" if statistics_saved else "FAILED", f"{statistics_saved} team-stat rows stored; {statistics_failures} provider failures")
    else:
        with SessionLocal() as db:
            existing_stats = int(db.scalar(select(func.count(TeamMatchStatistic.id)).join(Fixture, TeamMatchStatistic.fixture_id == Fixture.id).where(Fixture.provider == "api-football")) or 0)
        report("Historical/statistics detail", "SKIPPED", f"no fresh completed fixture; {existing_stats} existing real team-stat rows retained")
    ...
    odds_saved = 0
    odds_events_seen = 0
    if football.configured and not football_fixture_request_blocked:
        with SessionLocal() as db:
    ...
    if odds_provider is not None and odds_provider.configured and args.odds_sport_key:
        try:
            ...
            odds_rate_remaining = odds_provider.last_rate_limit_remaining
    ...
    if odds_rate_remaining is not None:
        report("The Odds API quota", "INFO", f"{odds_rate_remaining} request(s) remaining according to the provider response header")

    if args.live and not football_fixture_request_blocked:
        try:
        ...
        report("Live provider", "SKIPPED", "API-Football fixture request was blocked; no additional live call attempted")
    else:
    ...
    return 1 if football_fixture_request_blocked else 0
